...
func (s *nnrfService) HeartbeatnfInstance(ctx context.Context) (err error) {
	chfContext := s.consumer.Context()
	...
	var res *http.Response
	firstpatchitem := models.PatchItem{Op: models.PatchOperation_REPLACE,
		Path:  "/nfStatus",
		Value: models.NfServiceStatus_REGISTERED}
	patchitems = append(patchitems, firstpatchitem)
	_, res, err = client.NFInstanceIDDocumentApi.UpdateNFInstance(ctx, chfContext.NfId, patchitems)
	if err != nil || res == nil {
		logger.ConsumerLog.Errorf("CHF Update to NRF Error[%v]", err)
		time.Sleep(2 * time.Second)
	}
	defer func() {
		if resCloseErr := res.Body.Close(); resCloseErr != nil {
