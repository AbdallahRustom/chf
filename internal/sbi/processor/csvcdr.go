...
var (
	cdrWriterOnce sync.Once
	cdrWriter     *RotatingCSVWriter
	cdrWriterErr  error
)
...
// Call this from your main shutdown hook (SIGTERM) to finalize the file.
func CloseCDRWriter() {
	if cdrWriter != nil {
		_ = cdrWriter.Close()
	}
...
func dumpCdrToCSV(records []*cdrType.CHFRecord, action ChargingDataAction, timeStamp time.Time) error {
	w, err := getCDRWriter()
	if err != nil {
		return err
	}
	...
	for _, record := range records {
		...
		var actionType, startTimeStr, endTimeStr string
		switch action {
		case CDRCreate:
			actionType = "Create"
			startTimeStr = timeStamp.UTC().Format(time.RFC3339)
		case CDRUpdate:
			actionType = "Update"
		case CDRRelease:
			actionType = "Release"
			endTimeStr = timeStamp.UTC().Format(time.RFC3339)
		default:
		...
		for _, multiUnitUsage := range chfCdr.ListOfMultipleUnitUsage {
			...
			for _, details := range detailsList {
				...
				if err != nil {
					return fmt.Errorf("invalid UplinkVolume %q: %w", details.UplinkVolume, err)
				}
				downlinkVolume, err = strconv.ParseInt(details.DownlinkVolume, 10, 64)
				if err != nil {
					return fmt.Errorf("invalid DownlinkVolume %q: %w", details.DownlinkVolume, err)
				}
			}
		}
