...
func (s *Server) Run(traceCtx context.Context, wg *sync.WaitGroup) error {
	...
	timervalue := factory.ChfConfig.Configuration.HeartBeat - 1
	fmt.Println(timervalue)
	ticker := time.NewTicker(time.Duration(timervalue) * time.Second)
	done := make(chan bool)
	...
	go func() {
		...
		for {
			select {
			case <-ticker.C:
				err = s.Consumer().HeartbeatnfInstance(context.Background())
				if err != nil {
					log.Printf("CHF Cannot send HeartBeat[%s]", err.Error())
				}
			case <-done:
				return
			}
	...
	// err = s.Consumer().HeartbeatnfInstance(context.Background())
	// if err != nil {
	// 	logger.InitLog.Errorf("CHF Cannot send HeartBeat[%s]", err.Error())
	// }
