...
func handleSUR() diam.HandlerFunc {
	return func(c diam.Conn, m *diam.Message) {
		...
		filter_ueId := bson.M{"ueId": subscriberId}
		chargingInterface, err := mongoapi.RestfulAPIGetOne(chargingDatasColl, filter_ueId)
		if err != nil {
			logger.ChargingdataPostLog.Errorf("Get tarrif error: %+v", err)
		}
		if chargingInterface == nil {
			logger.ChargingdataPostLog.Warningf(
				"No ChargingData found for UE:[%+v]", subscriberId)
			return
		}
		if chargingInterface["ratingGroup"] == nil {
			queryStrength := 2
			chargingInterface["ratingGroup"] = int32(1)
			if _, err = mongoapi.RestfulAPIPutOne(
				chargingDatasColl, chargingInterface, chargingInterface, queryStrength); err != nil {
				logger.ChargingdataPostLog.Errorf("Fail to put charging data to mongoDB err: %+v", err)
			}
		}
